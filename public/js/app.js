angular.module("demoLab", ["ngFileUpload", "ngResource", "ngTable"]).controller("demoController", function($scope, $http, Upload, $resource, NgTableParams) {
      ...
      getData: function(params) {
        ...
        return Api.get(params.url()).$promise.then(function(data) {
        	setTimeout(function() {
        		...
        		if(data.list[i].photos) {
        		var arrayBufferView = new Uint8Array( data.list[i].photos.data.data );
    			var blob = new Blob( [ arrayBufferView ], { type: data.list[i].photos.contentType } );
    			var urlCreator = window.URL || window.webkitURL;
    			var imageUrl = urlCreator.createObjectURL( blob );
    			var img = document.querySelector( "#photo"+data.list[i]._id );
    			img.src = imageUrl;
    		}
    ...
    $scope.search = function(search) {
    	...
    	$http.get("/api/places/"+search).then(function(resp) {
	...
	$scope.update = function(document) {
		...
		if(document.country && document.country.text) document.country = document.country.text;
		$http.put("/api/places/"+document._id, document).then(function() {
   		    $scope.tableParams.reload();
    ...
    $scope.favorite = function(id, status) {

    	$http.post("/api/places/favorite",{ _id: id, status: status}).then(function() {
   		    $scope.tableParams.reload();
    ...
    $scope.delete = function(id) {
    	...
    	$http.post("/api/places/favorite/"+id,{ _id: id}).then(function() {
   		    $scope.tableParams.reload();
